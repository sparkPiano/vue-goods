<template>
    <div>
        <Table :ref="refs" :columns="columnsList" :data="thisTableData" :height="height" @on-row-dblclick="dblclick"
...
<script>
     ...
     * 设置属性
     */
    const setIntvButton = (vm, h, currentRow, index) => {
        return h('Button', {
            props: {
                type: 'info',
                size: 'small'
            ...
            on: {
                'click': () => {
                    vm.$emit('input', vm.handleBackdata(vm.thisTableData));
                    vm.$emit('on-setIntv', vm.handleBackdata(vm.thisTableData), index);
                }
            }
        }, '设置属性');
    };
                    ...
                    if (item.handle) {
                        item.render = (h, param) => {
                            ...
                            item.handle.forEach(item => {
                                ...
                                }else if (item === 'setIntv') {
                                    children.push(setIntvButton(this, h, currentRowData, param.index));
                                }
                            });
